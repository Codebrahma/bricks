...
const ShowStatus = styled(Absolute)`
  ...
  right: 10px;
  ${({ theme, variant }) =>
    applyVariation(theme, `${variant}.status`, 'corousel')}
`;
...
const CorouselContainer = styled(Relative)`
  ...
  overflow: hidden;
  ${({ theme, variant }) =>
    applyVariation(theme, `${variant}.container`, 'corousel')}
`;
    ...
    <CorouselContainer ref={coroselRef} variant={variant}>
      ...
      >
        {children.map(({ props: { children, img } }, i) => (
          <Slide key={i} img={img} position={contentPosition} variant={variant}>
...
Corousel.propTypes = {
  ...
  transitionTimer: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
